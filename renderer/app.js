...
/* Legacy overlay mode helpers — superseded by app-overlay-prefs.js when loaded */
...
async function applyOverlayMode(mode) {
  if (typeof applyOverlayInteract === "function") {
    await applyOverlayInteract(mode !== "passthrough");
    return;
  }
  ...
  if (window.electronAPI && window.electronAPI.overlayClickThrough) {
    try { await window.electronAPI.overlayClickThrough(clickThrough); } catch (_) {}
  }
}
...
function initOverlayMode() {
  if (typeof initOverlayInteract === "function") {
    initOverlayInteract();
    return;
  }
...
initResources().then(function () {
  ...
  if (typeof initOverlayInteract === "function") initOverlayInteract();
  else initOverlayMode();
  if (typeof initOverlayAppearance === "function") initOverlayAppearance();
  loadMissions();
});
