/**
 * If Game.log identifies the player as OLARIA, lock the entire UI.
 */
(function () {
  var locked = false;

  function ensureOverlay() {
    var el = document.getElementById("olaria-lock");
    if (el) return el;
    ...
    el.innerHTML =
      '<div style="font-size:1.75rem;font-weight:700;margin-bottom:0.75rem">Sorry, Olaria, you are muted.</div>' +
      '<div style="color:#8b949e;font-size:0.95rem;max-width:28rem;line-height:1.5">' +
      "This tracker will not work for you. The log identified your character as OLARIA.</div>";
  ...
  function applyLock(on) {
    ...
    if (locked) {
      ...
      // Block pointer events on the rest of the app
  ...
  window.checkOlariaLock = pollPlayer;
